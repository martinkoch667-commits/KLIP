...
/* bold gradient showcase — fillable hero image, floating brand posts, glossy emblems */
function HeroShowcase() {
  const thumbs = [
    { i: 0, brand: 'Maison Lou',  tag: 'L\u2019été se\nréserve', pos: { left: '5%',  top: '15%' },    w: 158, cls: 'floatA' },
    { i: 2, brand: 'Studio Vél',  tag: 'On recrute.',            pos: { right: '6%', top: '9%' },     w: 150, cls: 'floatB' },
    { i: 5, brand: 'Brut & Co',   tag: 'Édition\nlimitée',       pos: { left: '8%',  bottom: '7%' },  w: 146, cls: 'floatB' },
    { i: 1, brand: 'Café Oreste', tag: 'Nouvelle\ncarte ↗',      pos: { right: '7%', bottom: '8%' },  w: 152, cls: 'floatA' },
  ];
  return (
    <div className="grad-stage" style={{ height: 'clamp(380px, 47vw, 560px)' }}>
      {/* center hero post — drop your own photo */}
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: 'clamp(216px,23vw,288px)', zIndex: 5 }}>
        <div className="img-frame">
          <div style={{ aspectRatio: '4 / 5', position: 'relative' }}>
            <image-slot id="klip-hero-post" shape="rect" placeholder="Glissez une photo" style={{ display: 'block', width: '100%', height: '100%' }}></image-slot>
          </div>
          <div style={{ padding: '10px 13px 13px', display: 'flex', alignItems: 'center', gap: 9, background: 'var(--white)' }}>
            <span style={{ width: 26, height: 26, borderRadius: '50%', background: 'linear-gradient(140deg,var(--acid),#1f7a4d)', flexShrink: 0 }} />
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontFamily: 'var(--grotesk)', fontWeight: 800, fontSize: 12.5 }}>maisonlou</div>
              <div style={{ height: 5, width: '62%', borderRadius: 3, background: 'rgba(13,15,10,.12)', marginTop: 4 }} />
            </div>
            <Icon name="heart" size={16} style={{ color: 'var(--ink-3)' }} />
          </div>
        </div>
      </div>
      ...
      {thumbs.map((t, k) => (
        <div key={k} className={`hero-float ${t.cls}`} style={{ position: 'absolute', ...t.pos, width: t.w, zIndex: 3 }}>
          <PostThumb i={t.i} brand={t.brand} tag={t.tag} />
        </div>
      ))}

      {/* glossy 3D emblems */}
      <div className="hero-float" style={{ position: 'absolute', left: '29%', top: '6%', zIndex: 4 }}><Emblem3D shape="blob" tone="lime" size={84} float /></div>
      <div style={{ position: 'absolute', right: '30%', bottom: '6%', zIndex: 4 }}><Emblem3D shape="spark" tone="forest" size={70} float /></div>
      <div className="hero-float" style={{ position: 'absolute', right: '39%', top: '11%', zIndex: 4 }}><Emblem3D shape="drop" tone="teal" size={54} float /></div>
...
function Hero({ variant = 'split', onDemo }) {
  ...
  if (variant === 'centered') {
    return (
      <header id="top" className="section hero-grid" style={{ paddingTop: 150, paddingBottom: 30, textAlign: 'center', overflow: 'hidden', position: 'relative' }}>
        <div className="gblob" style={{ position: 'absolute', width: 'min(46vw,560px)', aspectRatio: '1', left: '-9%', top: '1%', borderRadius: '50%', background: 'radial-gradient(circle, rgba(95,230,180,.5), transparent 66%)', filter: 'blur(72px)', opacity: 'calc(var(--grad) * 1.15)', zIndex: 0, pointerEvents: 'none' }} />
        <div className="gblob" style={{ position: 'absolute', width: 'min(40vw,480px)', aspectRatio: '1', right: '-7%', top: '14%', borderRadius: '50%', background: 'radial-gradient(circle, rgba(200,241,53,.42), transparent 66%)', filter: 'blur(72px)', opacity: 'calc(var(--grad) * 1)', zIndex: 0, pointerEvents: 'none', animationDelay: '-6s' }} />
        <div className="wrap" style={{ position: 'relative', zIndex: 2 }}>
          <div className="reveal in" style={{ display: 'flex', justifyContent: 'center', marginBottom: 24 }}><span className="eyebrow">Le studio social des agences</span></div>
          <div className="reveal in d1" style={{ display: 'inline-block' }}>
            <h1 className="display upper" style={{ fontSize: 'clamp(46px, 8vw, 112px)' }}>
              Postez pour dix clients<br /><span className="it acid-text">comme pour un seul.</span>
            </h1>
          </div>
          <div className="reveal in d2" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            {sub}
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 34, alignItems: 'center', justifyContent: 'center' }}>
              <a href="#tarifs" className="btn btn-grad">Commencer gratuitement <Icon name="arrowUR" size={18} className="arr" /></a>
              <button className="btn btn-ghost" onClick={onDemo}><Icon name="play" size={16} /> Voir la démo</button>
            </div>
            {trust}
          </div>
        </div>
        <div className="wrap reveal in d3" style={{ position: 'relative', zIndex: 2, marginTop: 60 }}>
          <HeroShowcase />
        </div>
      </header>);
