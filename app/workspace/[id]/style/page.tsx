...
function toGoogleFontsUrl(font: string) {
...
export default function WorkspaceStylePage() {
  ...
  const loadFont = useCallback((font: GoogleFont) => {
    ...
    link.rel = "stylesheet";
    link.href = toGoogleFontsUrl(font);
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="flex items-center justify-between px-6 py-3.5 border-b border-[#E0E0E0] bg-white shrink-0">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-inter text-[#888]">{workspaceName}</span>
            <span className="text-[#DDD]">/</span>
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          <div className="w-[45%] border-r border-[#E0E0E0] flex flex-col overflow-y-auto shrink-0">
            <div className="p-7 flex-1">
              <h2 className="font-syne font-extrabold text-xl text-black mb-1">Style visuel</h2>
              <p className="text-sm font-inter text-[#888] mb-7">
                Définissez l&apos;identité visuelle de <span className="text-black font-medium">{workspaceName}</span>
              </p>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full bg-[#F5F5F5] border border-[#E0E0E0] rounded px-3 py-2.5 text-sm font-inter text-black focus:border-black outline-none transition-colors appearance-none cursor-pointer"
                        style={{ fontFamily: `"${s.font}", sans-serif` }}
                      >
                        {GOOGLE_FONTS.map((f) => (
                          <option key={f} value={f}>{f}</option>
                        ))}
                      </select>
                      <p className="text-[11px] font-inter text-[#AAA] mt-1.5">Police chargée depuis Google Fonts</p>
                    </div>
            ...
            {/* Save buttons */}
            <div className="sticky bottom-0 px-7 py-4 bg-white border-t border-[#E0E0E0] flex flex-col gap-2">
              <button
                onClick={handleSaveAndContinue}
                disabled={saving}
                className="w-full py-3 rounded bg-acid text-black text-sm font-inter font-bold hover:bg-acid/90 transition-colors disabled:opacity-50"
              >
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="w-full py-2.5 rounded border border-[#E0E0E0] bg-white text-sm font-inter font-medium text-[#555] hover:border-black hover:text-black transition-colors disabled:opacity-50"
              >
          ...
          <div className="flex-1 flex items-center justify-center bg-[#F5F5F5] p-8 overflow-auto">
            <div className="flex flex-col items-center gap-4">
              ...
              >
                ...
                <div style={logoPositionStyle(s.logoPosition, s.logoSize)}>
                  {s.logoUrl ? (
                    ...
                    <div className="w-full h-full rounded bg-white/80 border border-white/50 flex items-center justify-center">
              ...
              <div className="flex items-center gap-6 text-xs font-inter text-[#AAA]">
                <span>Police : <span className="text-black font-medium" style={{ fontFamily: `"${s.font}", sans-serif` }}>{s.font}</span></span>
