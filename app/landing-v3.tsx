      ...
      <div className="wrap" style={{ position: 'relative', zIndex: 2 }}>
        ...
        {prelaunch ? (
          ...
          <div className="h-intro hero-cta" style={{ display: 'flex', gap: 14, justifyContent: 'center', alignItems: 'center', marginTop: 34, flexWrap: 'wrap' }}>
            {/* On demande l'e-mail plutôt qu'« Essayer gratuitement » : il arrive
               pré-rempli dans la fenêtre d'inscription. Seul le CTA du hero
               change, celui de la nav ouvre toujours l'inscription. */}
            <CtaEmailHero />
            ...
            const inner = (
              <div style={{ position: 'relative', background: t.pop ? 'var(--forest)' : 'var(--card)', color: t.pop ? 'var(--cream)' : 'var(--ink)', borderRadius: 'var(--r)', padding: '30px 26px', border: t.pop ? 'none' : '1px solid var(--line)', boxShadow: t.pop ? '0 40px 80px -40px rgba(7,33,23,.7)' : '0 20px 44px -30px rgba(16,19,11,.2)' }}>
                ...
                <div style={{ fontFamily: 'var(--sans)', fontSize: 12, color: t.pop ? 'var(--cream-3)' : 'var(--ink-3)', marginBottom: 18, minHeight: 16 }}>
                  {launched
                    ? period === 'yearly'
                      // L'annuel se règle en une fois : on annonce la somme réellement
                      // débitée, puis le tarif des années suivantes.
                      ? tp('launchNoteYear', {
                          seats: LAUNCH_OFFER.seats,
                          firstYear: formatPrice(launchPrice(t.yearly * 12), locale),
                          full: formatPrice(t.yearly * 12, locale),
                        })
